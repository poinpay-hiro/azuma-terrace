...
7. **AI 生成のチラシは誤記を含み得る。Web 掲載用はレタッチで訂正し、印刷用は別に扱う（印刷版と Web 版で内容が違う）**: 2026-09-26、ハロウィン2026 のチラシ（AI 生成・オーナー未確認のまま作成）に誤記が3つあった（ガラポン「10:00」→ 正 11:00／「サンパ」→ 正 サンバ／「墨田区商店会連合会」→ 正 墨田区商店街連合会）。サイトの本文は訂正し、チラシ画像は Web 掲載用だけレタッチで訂正した（オーナー裁定 2026-09-26：訂正版は Web 用として採用・印刷には使わない）。
   - Why: 本文を直しても、チラシ画像の中の誤記は画像を差し替えるまで本番に出続ける（実際に push 後も画像内に誤記が残った）。書体を描き直すと周りの文字と字形が合わないことがあるので、**まず同じ画像の中にある同じ文字や部品（今回は「1」と「ド」の濁点）を写し、書体の再現は画像内に無い文字（今回は「街」だけ・ヒラギノ角ゴ W8）に限る**と自然に仕上がる。レタッチ範囲の外に手が入っていないことは、原本を同じ品質で保存し直しただけの場合と画素差が一致することで確かめられる。
   - How to apply:
     - イベントのチラシを受け取ったら、掲載前に日時・時間・団体名・催し名をデータ（または正しい情報）と突き合わせ、食い違いは直さずに報告してオーナーの裁定を待つ。
     - 原本フォルダ `~/Downloads/azuma-assets/` では、Web 用の訂正版を `…_web訂正版.jpg`、元の誤記入りを `…_誤記あり_印刷前要訂正.jpg` として**両方残す**（削除しない）。サイトの `assets/events/` と `og-events.jpg` は Web 訂正版から作る。
     - **⚠️ 印刷前に要訂正**: `_誤記あり_印刷前要訂正` のファイルは誤記を含むので、**そのまま印刷に使わない**。印刷する場合は、元のデザインデータで誤記を直してから刷る（Web 用のレタッチ版は印刷原稿として使わない＝オーナー裁定）。
   - 関係コミット: `5734756`（本文の訂正）／ops_orders id=23 のコミット（チラシ画像と OGP を Web 訂正版に差し替え）
